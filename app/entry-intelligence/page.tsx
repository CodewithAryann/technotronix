...
export default function PeopleCountingPage() {
  ...
  const nextBenefit = () => setCurrentBenefit((prev) => (prev + 1) % benefitsData.length);
  const prevBenefit = () =>
    setCurrentBenefit((prev) => (prev === 0 ? benefitsData.length - 1 : prev - 1));

  return (
    <main className="relative bg-[#f8f9fb] text-gray-800 overflow-hidden">
      {/* ===== HEAD FOR SEO ===== */}
      <Head>
        <title>ZAISENSE Store Entry Analytics | Technotronix UAE</title>
        <meta
          name="description"
          content="ZAISENSE Store Entry Analytics by Technotronix UAE transforms entry data into actionable insights. Optimize store performance, footfall, and customer experience in real time."
        />
        <link rel="canonical" href="https://technotronix.ae/people-counting" />
      </Head>
        ...
        <div className="absolute inset-0 w-full h-full">
          <AnimatePresence mode="wait">
            <motion.div
              ...
              aria-label={`ZAISENSE Entry Analytics Slide ${currentImage + 1}`}
            />
          </AnimatePresence>
        </div>
      ...
      <section className="max-w-6xl mx-auto px-6 sm:px-10 py-20 text-center">
        ...
        >
          ...
          <p className="text-lg leading-relaxed text-gray-600">
            First impressions start at the door — and so does your data. ZAISENSE Store Entry Analytics empowers retailers to understand visitor flow, traffic patterns, and engagement behavior the moment customers walk in. Using AI-powered vision technology, our system turns every store entry point into a source of valuable business intelligence.
          </p>
        </motion.div>
      </section>
      ...
      <section className="max-w-6xl mx-auto px-6 sm:px-10 py-20 text-center">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-8">
          {[
            { title: "AI-Precision Counting", desc: "Deep-learning algorithms ensure superior accuracy in people detection and differentiation.", img: "/images/entry-intell/ai.png" },
            { title: "Real-Time Dashboards", desc: "Get instant visibility into entry traffic, occupancy, and visitor flow trends.", img: "/images/entry-intell/real.png" },
            { title: "Seamless Integration", desc: "Works flawlessly with POS, ERP, and other VisiGz modules like Heatmap and Queue Management.", img: "/images/entry-intell/seamless.png" },
            { title: "Privacy-First Design", desc: "Fully anonymous data collection with no personal identifiers.", img: "/images/entry-intell/privacy.png" },
      ...
      <section className="max-w-4xl mx-auto px-6 sm:px-10 py-20 text-center">
          ...
          {benefitsTitles.map((title, i) => (
            <button
              ...
              className={`px-5 py-2 rounded-full font-semibold transition-all duration-300 ${
                i === currentBenefit ? "bg-[#004D94] text-white shadow-lg scale-105" : "bg-gray-200 text-gray-700 hover:bg-gray-300"
              }`}
            >
              {title}
            </button>
        ...
        <div className="relative">
          <button onClick={prevBenefit} className="absolute -left-6 top-1/2 -translate-y-1/2 bg-gray-200 hover:bg-gray-300 text-gray-700 p-3 rounded-full shadow z-10">‹</button>
          <button onClick={nextBenefit} className="absolute -right-6 top-1/2 -translate-y-1/2 bg-gray-200 hover:bg-gray-300 text-gray-700 p-3 rounded-full shadow z-10">›</button>
          <AnimatePresence mode="wait">
            <motion.div
              key={currentBenefit}
            ...
            >
              ...
              <div className="p-8 flex flex-col items-center text-center space-y-4">
                <h3 className="text-3xl sm:text-4xl font-semibold text-[#004D94]">{benefitsTitles[currentBenefit]}</h3>
                <p className="text-gray-700 text-lg max-w-2xl">{benefitsData[currentBenefit]}</p>
              </div>
            </motion.div>
          </AnimatePresence>
        </div>
      ...
      <section className="py-24 bg-gray-50">
        <div className="max-w-6xl mx-auto px-6 sm:px-10 text-center">
          <motion.h2 initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="text-3xl sm:text-4xl font-semibold text-[#022e64] mb-12">
            Business Impact & ROI
          </motion.h2>
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {serveIndustries.map(({ title, desc, icon: Icon }, i) => (
              ...
              >
                <motion.div whileHover={{ rotate: 360 }} transition={{ duration: 1 }} className="p-4 bg-[#022e64]/10 rounded-full text-[#022e64]">
                  <Icon size={36} />
                </motion.div>
                <h3 className="text-xl font-semibold text-[#022e64]">{title}</h3>
